...
class update_user_info {
    ...
    public function StoreUserInfo($name, $email, $password, $gender, $age) {
        ...
        $stmt = $this->conn->prepare("INSERT INTO user_information(name, email, encrypted_password, salt, gender, age) VALUES(?, ?, ?, ?, ?, ?)");
        ...
        if ($result) {
            $stmt = $this->conn->prepare("SELECT name, email, encrypted_password, salt, gender, age FROM user_information WHERE email = ?");
    ...
    public function CheckExistingUser($email) {
        $stmt = $this->conn->prepare("SELECT email from user_information WHERE email = ?");
